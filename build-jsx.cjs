...
(function () {
  ...
  function App() {
    ...
    var sf = useState(function () {
      try { var s = localStorage.getItem('bi.statusFilter') || 'realizado'; return [s]; }
    ...
    useEffect(function () {
      // Salva string simples — index.html le antes do bundle e passa pra _makeBit que so aceita string
      var sfStr = Array.isArray(statusFilter)
        ? (statusFilter.length === 1 ? statusFilter[0] : 'tudo')
        : (statusFilter || 'realizado');
      try { localStorage.setItem('bi.statusFilter', sfStr); } catch (e) {}
